...
class STM32Flasher(object):
    ...
    def reset(self):
        #Sends an CMD_ERASE to the bootloader
        self.serial.flushInput()
        self.serial.write(self._create_cmd_message((CMD_RESET,)))
        data = self.serial.read(1)
        if len(data) == 1:
            if struct.unpack("b", data)[0] != ACK:
                raise ProgramModeError("CMD_RESET failed")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--with-reset', dest='reset', action='store_const',
                        const=True, default=False,
                        help='With reset on write finish (default: sequential)')
